...
public class HeapSort {
	public static void main(String[] args) {
	    ...
	    heapSort(a);
		System.out.println("Max_Heapify: " + Arrays.toString(a));
	 ...
	 * 堆排序
	 *
	 * 1. 先将初始序列K[1..n]建成一个大顶堆, 那么此时第一个元素K1最大, 此堆为初始的无序区. 2. 再将关键字最大的记录K1 (即堆顶,
	 * 第一个元素)和无序区的最后一个记录 Kn 交换, 由此得到新的无序区K[1..n−1]和有序区K[n],
	 * 且满足K[1..n−1].keys⩽K[n].key 3. 交换K1 和 Kn 后, 堆顶可能违反堆性质, 因此需将K[1..n−1]调整为堆.
	 * 然后重复步骤②, 直到无序区只有一个元素时停止.
	 * 
	 * @param arr
	 *            待排序数组
	 */
	...
	private static void max_heapify(int[] arr, int limit) {//最大堆调整
